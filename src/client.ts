...
function getBaseUri(server: Server = 'default', config: Configuration): string {
  if (config.environment === Environment.Production) {
    if (server === 'default') {
      return 'https://demo.traccar.org/api';
    }
  }
  if (config.environment === Environment.Environment2) {
    if (server === 'default') {
      return 'https://demo2.traccar.org/api';
    }
  }
  if (config.environment === Environment.Environment3) {
    if (server === 'default') {
      return 'https://demo3.traccar.org/api';
    }
  }
  if (config.environment === Environment.Environment4) {
    if (server === 'default') {
      return 'https://demo4.traccar.org/api';
    }
  }
  if (config.environment === Environment.Environment5) {
    if (server === 'default') {
      return 'https://server.traccar.org/api';
    }
  }
  if (config.environment === Environment.Environment6) {
    if (server === 'default') {
      return pathTemplate`http://${new SkipEncode(config.host)}:${new SkipEncode(config.port)}/api`;
    }
  }
